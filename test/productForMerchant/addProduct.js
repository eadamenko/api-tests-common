describe('Add Product |', function () {
    ...
    require('../../apiHelper/getAdminToken');
    ...
    describe('When merchant add product |', function () {
        ...
        it('should be an error if there is customer token', function (done) {
            chai.request(app)
                .post(url)
        ...
        it('should be an error if there is admin token', function (done) {
            chai.request(app)
                .post(url)
                .set('X-Authorization', 'Bearer ' + process.env.TOKEN_ADMIN) //invalid role token
                .send({
                    name: 'producttest',
                    description: 'mpollich',
                    price: 1000,
